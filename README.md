# ![rayquaza](https://media.tenor.com/o7-lGxrrr6UAAAAj/rayquaza-pok%C3%A9mon.gif) POKEMON PINBALL GBC ![rayquaza](https://media.tenor.com/o7-lGxrrr6UAAAAj/rayquaza-pok%C3%A9mon.gif)

# ✨Description ![evee](https://media.tenor.com/sj3IHPPDOxEAAAAi/eevee-eevee-evolution.gif)

Recreation of Pokemon Pinball for the Game Boy. This recreation is made on a surface level compared with the original one, meaning that many features are not included, as more than one map.

This recreation includes:

- Flippers and launcher for the ball
- Score system, with high score and last score
- Life system, which resets the game once you lost all the lifes
- Collision system with all the pokemons
- Debug features
- Repository URL: [REPOSITORY](https://github.com/abelnunez5/Pokemon_Pinball_2D)

# ✨Members ![umbreon](https://media.tenor.com/Zsu1LBqCXdMAAAAi/eeveelution-eevee-evolution.gif)

- ### [Alejandro](https://github.com/Alejandrorg5)
- ### [Abel](https://github.com/abelnunez5)
- ### [Pol](https://github.com/Lexpanpo)
- ### [Andrea](https://github.com/veawenn)
- ### [Hugo](https://github.com/HuggoESC)

# ✨Project Features ![Jolteon](https://media.tenor.com/0bIjwnKhaOcAAAAj/jolteon-pokemon-jolteon.gif)

- Game engine: Raylib, Box2D

- Programming language: C#, C++

- Visual style: Pixel art faithful to the original

- Physics and game mechanics inspired by the original game

- Game replicated from the GBC (Game Boy Color) version

- Sound and music based on the classic version

- Controls adapted for keyboard

 
# ✨Controls ![Espeon](https://media.tenor.com/cERJ7M_mH-cAAAAj/eeveelution-eevee-evolution.gif)

- **A/D** move Flippers
- **SPACEBAR** Charge ball
- **ENTER** Skyp intro



# ✨Debug Mode ![Glaceon](https://media.tenor.com/PB_v8oFCde4AAAAi/eeveelution-eevee-evolution.gif)

 - {Dev01} - {Role(s)}
 - {Dev02} - {Role(s)}
 - {Dev03} - {Role(s)}
 - {Dev04} - {Role(s)}

# ✨Resources ![Leafeon](https://media.tenor.com/2DdMTd9WbCkAAAAi/eeveelution-eevee-evolution.gif)

- [Sprites](https://www.spriters-resource.com/)

## License

This project is licensed under an unmodified MIT license, which is an OSI-certified license that allows static linking with closed source software. Check [LICENSE](LICENSE) for further details.

{AdditionalLicenses}
